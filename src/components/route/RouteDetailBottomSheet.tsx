...
export default function RouteDetailBottomSheet({ route, startLabel, destLabel }: Props) {
  const {
    totalTime,
    payment,
    busTransitCount,
  ...
  const iconByType = (t: number) =>
    t === 1 ? <FaSubway size={14} color="#5C9EFF" /> :
    t === 2 ? <FaBus    size={14} color="#FFB200" /> :
              <FaWalking size={14} color="#A0A4A8" />;

  const labelBySubPath = (s: SubPath) =>
    s.trafficType === 1 ? s.lane?.[0]?.name ?? "지하철"
    : s.trafficType === 2 ? s.lane?.[0]?.busNo ?? "버스"
    : "도보";
        ...
        <Sheet.Content disableDrag>
          <Sheet.Scroller style={{ padding: "22px 20px 32px" }}>
            ...
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "#555" }}>
              환승 {Number(busTransitCount || 0) + Number(subwayTransitCount || 0)}회&nbsp;|&nbsp;
              {payment.toLocaleString()}원
            </p>

            {/* ── ② 진행 막대 ─────────────────────────── */}
            <RouteProgressBar subPaths={route.subPath} />
            <hr style={{ border: "none", borderTop: "1px solid #E5E5E5", marginTop: 18 }} />
            ...
            <div style={{
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                <img src="/assets/start-pin.png" alt="출발" width={24} height={24} />
              </div>
              <div>
                    ...
                    {s.passStopList?.stations?.length && (
                      <ul style={{ paddingLeft: 16, margin: "6px 0 0" }}>
                        {s.passStopList.stations.map(st => (
                          <li key={st.stationID} style={{ fontSize: 12, color: "#555", lineHeight: 1.35 }}>
                            {st.stationName}
                          </li>
                        ))}
                      </ul>
              ...
              {/* 도착지 */}
              <div style={{ display: "flex", justifyContent: "center" }}>
                <img src="/assets/end-pin.png" alt="도착" width={24} height={24} />
              </div>
              <div>
